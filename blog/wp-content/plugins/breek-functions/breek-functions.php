<?php
/**
 * Plugin Name: Breek Theme Functions
 * Plugin URI: http://estudiopatagon.com
 * Description: This plugin enables core functions for <a href="http://estudiopatagon.com/themes/wordpress/breek">Breek - High Speed Masonry Theme for Wordpress</a> by <a href="http://estudiopatagon.com/">EstudioPatagon</a>.
 * Version: 1.4.0
 * Author: Estudio Patagon
 * Author URI: http://estudiopatagon.com
...
defined( 'ABSPATH' ) or die( 'No script kiddies please!' ); // No direct access allowed
 ...
 if ( !class_exists('epcl_breek_theme_functions') ) {
    ...
    class epcl_breek_theme_functions {
        ...
        public function __construct() {
            ...
            require_once(EPCL_PLUGIN_PATH.'/widgets.php');
            require_once(EPCL_PLUGIN_PATH.'/metaboxes.php');
		...
		public function posts_columns($defaults){
            $defaults[EPCL_THEMEPREFIX.'_post_image'] = esc_html__('Image', 'breek');
			return $defaults;
		}
            ...
            if( isset($epcl_theme['enable_lazyload_posts']) && $epcl_theme['enable_lazyload_posts'] === '1' ){
                $content = preg_replace_callback( '/<img .*?>/', function($matches) {
                    return preg_replace( '/\bsrc\s*=\s*[\'"](.*?)[\'"]/', 
                        'src="'.EPCL_THEMEPATH.'/assets/images/transparent.gif" data-lazy="true" data-src="$1"', $matches[0] ); } , $content);
            }
	 ...
	 new epcl_breek_theme_functions();
