...
function epcl_generate_custom_styles(){
	...
	if( $epcl_theme['background_type'] == 3 && $epcl_theme['bg_body_full']['url'] )
        $css = 'body{ background: url('.$epcl_theme['bg_body_full']['url'].') no-repeat top center fixed !important; }';
    ...
    // Header menu links
    if( $epcl_theme['header_menu_link_color']['regular'] != $white ){
    ...
    // Content links
    if( $epcl_theme['content_link_color']['regular'] != $text_color ){
    ...
    if( $epcl_theme['button_secondary_bg_color'] != $secondary_color ){
        $css .= '.epcl-button.green, #single #comments.hosted nav.pagination a{ 
    ...
    // Tag color
    if( $epcl_theme['tag_text_color']['regular'] != $white ){
    ...
    if( $epcl_theme['tag_bg_color']['regular'] != $red ){
    ...
    if( $epcl_theme['sidebar_link_color']['regular'] != $text_color ){
