...
class UsersController extends Controller
{
    ...
    $this->validate($request, [
        ...
        if(Auth::attempt(['email' => $request->get('email') , 'password' => $request->get('password')])){
            ...
            return redirect()->back()->withErrors(['massage' => 'Wrong identify or password or email']);
        }
    }
    ...
    public function quickRegistration(Request $request){
        ...
        if($this->user->create($request->all())){
            /*if($this->sendRegistrationMassages($request->all())){
                if($this->user->where('email' , $request->get('email'))->update(['password' => bcrypt(\Session::get('temp_pass'))])){
    ...
    public  function  completedOrders(){
        if(Auth::user()->role_id ==1){
            ...
            $order = $this->user->join('orders', 'users.id', '=', 'orders.user_id')->where('order_efit_status' , '=' , 'done')->get();
        }
         else if(Auth::user()->role_id == 3){
    ...
    public function incomplateOrders(){
        if(Auth::user()->role_id ==1){
           ...
           $order = $this->user->join('orders', 'users.id', '=', 'orders.user_id')->where('order_status' , '=' , 'cancel')->where('order_efit_status' , '=' , '')->get();
          ...
          $order = $this->order->where('order_status' , '=' , 'cancel')->where('order_efit_status' , '=' , '')->where('user_id' , Auth::user()->id)->get();
    ...
    public function remindInvoice($id){
        ...
        if($this->sendInvoiceRemindEmail($data  , $email_name)){
            return redirect()->back()->with('success', 'The invoice sendet');
        } else {
            return redirect()->back()->withErrors(['massage' => 'Cant send the invoice']);
        }
    }
    ...
    public function sendInvoice(Request $request){
        ...
        $plan =  $this->plan->where('id' , $request->get('plan_name'))->first();
    ...
    public function supportMassage(Request $request){
        if($this->sendSupportEmail($request->all())){
            return redirect()->back()->with('sample_report' , 'Your massage sended');
        }else{
 ...
 public function webDevelopmentPost( Request $request)
  ...
  /*  public function getSampleReportName($files)
    ...
    public function sendRequestEmail($order , $user){
        ...
        $data['user'] = $user->email;
        $data['order'] = $order;
        $data['email_name'] = 'Order details submitted successfully Order ID :'.$order->id.''  ;
        $data['email_type'] = 'request_order';
        $data['email_subject'] = 'Order details submitted successfully Order ID : '.$order->id.'';
        if($this->sendEmail($data) == 0){
            return true;
        }
        else{
