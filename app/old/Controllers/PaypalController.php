...
class PaypalController extends Controller
{
    ...
    public function __construct(\Mailchimp $mailchimp , Paypal_data $paypal_data , User $user, Order $order , Invoice $invoice )
    {
        ...
        $this->_apiContext->setConfig(array(
            'mode' => 'live',
            'service.EndPoint' => 'https://www.paypal.com/cgi-bin/webscr',
    ...
    public function getCheckout(Request $request)
    {
        if($request->get('form_action') == 'login'){
            ...
            if (\Session::has('discount')) {
                ...
                \Session::put('invoice_no', $this->randomInvoice());
                foreach ($cart->items as $oldCart) {
                    $data['items'][$j++] =
                        [
                            'name' => $oldCart['item']['plan_name'],
 		...
 		foreach ($cart->items as $car) {
            ...
            if ($this->sendRegistrationMassages($request->all())) {
                if ($this->user->create(['email' => $request->get('email'), 'password' => bcrypt(\Session::get('temp_pass')), 'identifycator' => \Session::get('temp_id'), 'role_id' => 4])) {
                    /*for ($i = 0; $i < count(\Session::get('massive_to_pay')['plan_name']); $i++) {
                        $data['items'] = [
                            'name' => \Session::get('massive_to_pay')['plan_name'][$i]
                        ];
                    }*/
    ...
    public function getDone(Request $request)
    {
        ...
        if(\Session::has('invoice_no')){
        	...
        	->update(['order_status' => 'payed' , 'order_id' => $this->randomInvoice()]);
    ...
    public function getCancel(Request $request)
    {
        ...
        if(\Session::has('invoice_no')){
        	...
        	$this->order->where('order_id' , \Session::get('invoice_no'))->update(['order_status' => 'cancel' , 'order_id' => $this->randomInvoice() ]);       
        }
